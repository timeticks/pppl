...
public class AppEvtMgr //游戏的事件管理器
{
    ...
    public void RequestSendNotice(EvtItemData eventType) //主动请求查看某事件是否发生
    {
        ...
        switch (eventType.EventType)
        {
            ...
            case EvtType.ProductFinish:
            {
                ...
                {
                    ProduceAccessor produceAc = PlayerPrefsBridge.Instance.GetProduceAccessor();
                    List<int> finishList = produceAc.GetFinishRecipeList();
                    SendNotice(new EvtItemData(EvtType.ProductFinish), finishList);
                }
                break;
            }
            case EvtType.PrestigeTaskFinish:
            {
                if (PlayerPrefsBridge.Instance.GetPrestigeTaskFinish())
                {
                    SendNotice(new EvtItemData(EvtType.PrestigeTaskFinish));
                }
                break;
            }
            ...
            {
                if (PlayerPrefsBridge.Instance.GetTravelNotFinishEvent())
                {
    ...
    {
        if (HandlePool.Count == 0) return;
        mKeyList.Clear();
        foreach (var temp in HandlePool)
        {
            mKeyList.Add(temp.Key);
        }
        for (int i = 0; i < mKeyList.Count; i++)  //不能改为foreach，因为会方法中会移除HandlePool元素！！
        {

...
public class TaskDataMgr
{
    ...
    public void RemoveTaskInDic(int id , TaskStateType state) 
    {
        ...
        else { TDebug.LogErrorFormat("m_TaskDataDic不存在此任务{0}    {1}" , state,id); }
        if (m_TaskDict.ContainsKey(id))
        {
            m_TaskDict.Remove(id);
        }
        else { TDebug.LogErrorFormat("m_TaskDict不存在此任务{0}    {1}", state, id); }
    }
    ...
    public void AddTaskInDic(TaskDataBase task, TaskStateType state)
    {
        ...
        else { TDebug.LogErrorFormat("m_TaskDataDic不存在此任务{0}    {1}", state, task.m_Id); }
        if (!m_TaskDict.ContainsKey(task.m_Id))
        {
            m_TaskDict.Add(task.m_Id, state);
        }
        else { TDebug.LogErrorFormat("m_TaskDict不存在此任务{0}    {1}", state, task.m_Id); }
    }
    ...
    public TaskDataMgr(int[] taskIDs , int[] taskStates, List<int[]> askParam  , TaskMgr taskMgr)
    {
        ...
        if (taskIDs != null)
        {
            ...
            {
                TDebug.LogErrorFormat("任务参数错误:{0}   {1}   {2}"  , taskIDs.Length ,taskStates.Length ,askParam.Count);
            }
    ...
    public TaskDataBase GetTask(int taskId)
    {
        ...
        else { TDebug.LogErrorFormat("m_TaskDic不存在此任务   {0}",taskId); }
        return null;
    }

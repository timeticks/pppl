...
public class CircleImage : BaseImage {
    ...
    protected override void OnPopulateMesh(VertexHelper vhelper)
    {
        ...
        if (fill) //圆形
        {
            ...
            {
                vhelper.AddTriangle(vIdx, 0, vIdx+1);
            }
            if (fillPercent == 1)
            {
                //首尾顶点相连
                vhelper.AddTriangle(verticeCount - 1, 0, 1);
            }
        }
        else//圆环
        {
            ...
            {
                vhelper.AddTriangle(vIdx+1, vIdx, vIdx+3);
                vhelper.AddTriangle(vIdx, vIdx + 2, vIdx + 3);
            }
            if (fillPercent == 1)
            {
                //首尾顶点相连
                vhelper.AddTriangle(verticeCount - 1, verticeCount - 2, 1);
                vhelper.AddTriangle(verticeCount - 2, 0, 1);
            }
        }
